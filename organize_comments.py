        ...
        if replies:
            for i in range(0, len(replies)):
                ...
                if replies[i]["reply_control"].setdefault("sub_reply_entry_text"):
                    ...
                    l = [replies[i]["member"]["mid"], replies[i]["member"]["uname"], replies[i]["content"]["message"],a[0],replies[i]["like"]]
                else:
                    l = [replies[i]["member"]["mid"], replies[i]["member"]["uname"], replies[i]["content"]["message"],0,replies[i]["like"]]
